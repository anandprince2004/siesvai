...
def translate_query_for_retrieval(groq_client: Groq, question: str) -> str:
    ...
    for retrieval. The knowledge base and embedding model are English-
    centric, so Romanized Hindi/Marathi/Tamil queries often fail to
    retrieve the right chunks even when the LLM understands them fine.
...
def strip_reasoning_tags(text: str) -> str:
    """
    Some models emit an internal chain-of-thought wrapped in
    <think>...</think> before the actual answer. Users should only see
    the final answer, not the model's internal reasoning.
    """
    if "<think>" in text:
        if "</think>" in text:
...
def answer_question(groq_client: Groq, embedding_model, collection, question: str) -> dict:
    """
    Run the full pipeline for a single question: translate for retrieval,
    fetch context, generate the answer. Returns a dict with the answer
    and some debug info (useful for the API response and for logging).
    """
